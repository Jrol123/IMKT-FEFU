...
int main()
{
    ...
    MassCreator(imNum2, '\377', iCount, iSubCount, iLength);
    imNum2[0] = iCount;

    int iMaxIndex = max(imNum1[0], imNum2[0]);
    ...
    imNum1[0] = (imNum1[iMaxIndex+ 1] > 0) ? iMaxIndex + 1 : iMaxIndex;

    for (int iIndex = imNum1[0]; iIndex > 0; iIndex -= 1)
    {
        int iNum = imNum1[iIndex];
        string sFinal = "";
        ...
        for(int i = 4; i > 0; i --)
        {
            int t = pow(10, i - 1);
            int iStrNum = iNum / t;
            if (!(iStrNum == 0 && bStateOfZero && iIndex == imNum1[0]))
            {
